...
export default function WhatsAppButton({
  ...
  const message = productName
    ? t('mensaje', { nombre: productName, url: productUrl || '' })
    : 'Hola, me gustaría saber más sobre sus productos.'

  const href = `https://wa.me/${number}?text=${encodeURIComponent(message)}`

  const base =
    'inline-flex items-center gap-2 bg-[#25D366] text-white font-medium hover:bg-[#20BD5A] transition-colors'
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className={`${base} px-4 py-2 rounded-xl text-sm`}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
